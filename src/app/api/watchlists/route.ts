...
export async function GET() {
  try {
    ...
    const items = await db.select().from(watchlists).where(eq(watchlists.userId, currentUser.id));
    let liveTickers: LiveTicker[];
    try { liveTickers = await getLiveTickers(); } catch { liveTickers = []; }
    const hydrated = items.map((w) => {
      const asset = liveTickers.find((a) => a.symbol === w.symbol);
      return { ...w, currentPrice: asset?.currentPrice || 100, change24h: asset?.change24h || 0, trendStatus: asset?.trendStatus || "NEUTRAL" };
    });
...
export async function POST(request: Request) {
  try {
    ...
    const [item] = await db.insert(watchlists).values({
      id: newId, userId: currentUser.id, symbol, name: name || asset?.name || symbol,
      market: market || asset?.market || "crypto", alertHigh: alertHigh ? String(alertHigh) : null,
      alertLow: alertLow ? String(alertLow) : null,
      aiSentiment: asset?.trendStatus === "STRONG_BUY" || asset?.trendStatus === "BUY" ? "BULLISH" : "NEUTRAL",
      aiScore: asset?.aiConfidence || 80, favorite: true, notes,
    }).returning();
    return NextResponse.json({ success: true, item, message: `Added ${symbol} to watchlist` });
  } catch (error: any) {
