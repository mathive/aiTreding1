...
export async function POST(
  ...
  try {
    ...
    const closePercent = body.closePercent || 100; // 100% full close, or 25/50/75 partial
    ...
    const exitPrice = customExitPrice
      ? Number(customExitPrice)
      : asset
      ? asset.currentPrice
    ...
    if (portion >= 0.99) {
      ...
      const [res] = await db
        ...
        .set({
          status: "CLOSED",
          exitPrice: String(exitPrice.toFixed(4)),
          currentPrice: String(exitPrice.toFixed(4)),
          pnl: String(realizedPnl.toFixed(2)),
      ...
      // Partial close: reduce remaining amount & quantity
      ...
      const [res] = await db
        ...
        .set({
          ...
          quantity: String(remainingQuantity.toFixed(6)),
          currentPrice: String(exitPrice.toFixed(4)),
          notes: `${trade.notes ? trade.notes + " | " : ""}Partially closed ${closePercent}% at $${exitPrice.toFixed(2)} (PnL: ${realizedPnl >= 0 ? "+" : ""}$${realizedPnl.toFixed(2)})`,
          updatedAt: new Date(),
        })
        .where(eq(trades.id, trade.id))
        .returning();
    ...
    // Return margin + realized PnL back to user's balance
    ...
    await db.insert(notifications).values({
      ...
      title: `${realizedPnl >= 0 ? "🎉 Position Closed in Profit" : "📉 Position Closed"}`,
      message: `Closed ${portion >= 0.99 ? "100%" : `${closePercent}%`} of ${trade.symbol} (${trade.type}) at $${exitPrice.toFixed(2)}. Net PnL: ${realizedPnl >= 0 ? "+" : ""}$${realizedPnl.toFixed(2)} (${pnlPercent.toFixed(2)}%).`,
      type: realizedPnl >= 0 ? "tp_hit" : "sl_hit",
      isRead: false,
      metadata: { tradeId: trade.id, symbol: trade.symbol, pnl: realizedPnl, pnlPercent },
    });
    ...
    return NextResponse.json({
      ...
      message: `Closed ${trade.symbol} position. Net PnL: ${realizedPnl >= 0 ? "+" : ""}$${realizedPnl.toFixed(2)}`,
    });
  } catch (error: any) {
