...
export const ConnectionGate: React.FC<Props> = ({ onConnected }) => {
  ...
  const connect = async () => {
    ...
    try {
      ...
      if (!r.ok || !d.success) { setConnecting(false); setError(d.error||"Connection failed. Is MT5 running?"); return; }
      ...
      onConnected(bootData);
    } catch { setConnecting(false); setError("Bridge unreachable. Run: cd mt5-bridge && python server.py"); }
  };
        ...
        <div className="rounded-2xl bg-slate-900/90 border border-slate-800 shadow-2xl overflow-hidden">
          <div className="p-6 space-y-4">
            <div className="p-3 rounded-xl bg-amber-950/20 border border-amber-900/40 text-xs text-amber-300 flex items-start gap-2">
              ...
              <div>1. Open MT5 and log into Vantage<br/>2. Start bridge: <code className="bg-slate-950 px-1 py-0.5 rounded text-[11px] text-amber-200">cd mt5-bridge && python server.py</code></div>
            </div>
            {stored.length>0 && (
              <div className="space-y-1.5">
                <label className="text-[10px] uppercase font-bold text-slate-500">Saved Accounts</label>
                <div className="grid grid-cols-2 gap-2">
                  {stored.map((a:any)=>(<button key={a.login} onClick={()=>{setLogin(String(a.login));setServer(a.server);setError("");}} className={`p-2 rounded-lg text-left border cursor-pointer text-xs ${String(a.login)===login?"bg-red-950/40 border-red-700 text-red-200":"bg-slate-950 border-slate-800 text-slate-400 hover:text-white"}`}><div className="font-mono font-bold">#{a.login}</div><div className="text-[10px] text-slate-500 truncate">{a.server}</div></button>))}
                </div>
              </div>
            )}
            <div className="space-y-3">
              <div><label className="block text-[11px] font-semibold uppercase text-slate-400 mb-1">Account #</label><input type="number" value={login} onChange={e=>{setLogin(e.target.value);setError("");}} disabled={connecting} className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-2.5 text-sm font-mono text-white focus:outline-none focus:border-red-500 disabled:opacity-50"/></div>
              <div><label className="block text-[11px] font-semibold uppercase text-slate-400 mb-1">Password</label><input type="password" value={pwd} onChange={e=>{setPwd(e.target.value);setError("");}} disabled={connecting} className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-2.5 text-sm font-mono text-white focus:outline-none focus:border-red-500 disabled:opacity-50"/></div>
              <div><label className="block text-[11px] font-semibold uppercase text-slate-400 mb-1">Server</label><select value={server} onChange={e=>setServer(e.target.value)} disabled={connecting} className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-2.5 text-sm text-white cursor-pointer"><option>VantageFX-Live</option><option>VantageFX-Demo</option><option>VantageMarkets-Live</option><option>VantageMarkets-Demo</option></select></div>
            </div>
            ...
            <button onClick={connect} disabled={connecting} className="w-full py-3 bg-gradient-to-r from-red-600 to-rose-700 hover:from-red-500 hover:to-rose-600 disabled:from-red-900 rounded-xl font-bold text-sm uppercase text-white shadow-xl cursor-pointer flex items-center justify-center gap-2">
              {connecting ? <><div className="w-4 h-4 rounded-full border-2 border-white border-t-transparent animate-spin"/>Connecting...</> : <><Server className="w-5 h-5"/>Connect to Vantage MT5</>}
            </button>
          </div>
